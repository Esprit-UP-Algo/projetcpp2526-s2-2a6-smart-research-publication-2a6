/* =========================================================
   SMARTVISION (Oracle قديم) - 6 TABLES + SEQUENCES + TRIGGERS
...
CREATE TABLE PROJETS (
  ...
  CONSTRAINT CK_PRJ_DOM CHECK (
    DOMAINE_RECHERCHE IN ('Génomique','Protéomique','Développement de médicaments','Biologie moléculaire','Immunologie')
  ),
  CONSTRAINT CK_PRJ_STAT CHECK (STATUT IN ('En cours','Terminé','Suspendu','En attente')),
...
CREATE TABLE EQUIPEMENTS (
  ...
  RESPONSABLE_ID             NUMBER,
  CONSTRAINT CK_EQ_STAT CHECK (STATUT IN ('Actif','Hors service','Archivé')),
...
CREATE TABLE ECHANTILLONS (
  ...
  CONSTRAINT CK_ECH_TYPE CHECK (TYPE_ECHANTILLON IN ('ADN','ARN','Protéine','Cellule','Tissu','Organisme')),
...
CREATE TABLE EXPERIENCES (
  ...
  CONSTRAINT CK_EXP_STAT CHECK (STATUT IN ('En cours','Réussie','Échouée','Concluante','Archivée')),
...
CREATE TABLE PUBLICATIONS (
  ...
  CONSTRAINT CK_PUB_STAT  CHECK (STATUT IN ('Soumise','Acceptée','Publiée','Rejetée','Brouillon')),
...
-- 2) PUBLICATIONS: mots-clés + tri impact factor
DEFINE p_kw = '';
...
ORDER BY CASE STATUT WHEN 'Actif' THEN 1 WHEN 'Hors service' THEN 2 WHEN 'Archivé' THEN 3 ELSE 9 END,
