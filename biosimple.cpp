...
// ===================== Dialog: Error (Modern Design) =====================
// Error dialog matching the app theme
class ErrorDialog : public QDialog
{
public:
    ErrorDialog(QStyle* st, const QString& errorTitle, const QString& errorMessage, QWidget* parent=nullptr)
        : QDialog(parent)
    {
        setModal(true);
        ...
        QFrame* head = new QFrame;
        head->setFixedHeight(46);
        head->setStyleSheet(QString("background:%1; border-radius: 12px;").arg("#B14A4A")); // Error red color
        QHBoxLayout* hl = new QHBoxLayout(head);
        hl->setContentsMargins(12,8,12,8);

        QLabel* ic = new QLabel;
        ic->setPixmap(st->standardIcon(QStyle::SP_MessageBoxCritical).pixmap(18,18));

        QLabel* t = new QLabel(errorTitle);
        QFont ft = t->font(); ft.setBold(true); ft.setPointSize(11);
        t->setFont(ft);
        ...
        QLabel* msg = new QLabel(errorMessage);
        msg->setStyleSheet("color: rgba(0,0,0,0.65); font-weight: 800;");
        msg->setWordWrap(true);
        ...
        QPushButton* ok = new QPushButton(st->standardIcon(QStyle::SP_DialogApplyButton), "  Compris");
    ...
    : QMainWindow(parent)
{
    ...
    projDomain->addItems({"Domaine", "Biotech", "Recherche", "Qualité", "Santé", "Laboratoire", "Génomique", "Protéomique", "Pharmacologie", "Immunologie"});
    ...
    QLineEdit* projFinancing = new QLineEdit;
    projFinancing->setPlaceholderText("Ex. : ANR, CNRS, Privé, Budget interne...");
    projFinancing->setMaxLength(150);
    ...
    p2RightL->addWidget(projRow(QStyle::SP_ComputerIcon, "Source de financement", projFinancing));
...
QPushButton:hover{ background: %2; }
    ...
    QRegExp doiRegex("^[0-9]*$");
    leDOI->setValidator(new QRegExpValidator(doiRegex, leDOI));
    ...
    QLabel* empTotalLbl = new QLabel("repartition des employee par roles");
    ...
    QObject::connect(projSave, &QPushButton::clicked, this, [=](){
        // Collect form data into a ProjetRecord
        ProjetRecord proj;
        proj.nomDuProjet = projName->text().trimmed();
        proj.domaineDeRecherche = projDomain->currentText();
        proj.statut = projStatus->currentText();
        proj.nomDuResponsable = projOwner->text().trimmed();
        proj.budget = projBudget->value();
        proj.dateDeDebut = projStart->date();
        proj.dateDeFin = projEnd->date();
        proj.sourceDeFinancement = projFinancing->text().trimmed();

        // Create a GestProjCrud instance for validation
        GestProjCrud crud;
        QString error;
        
        // Validate the project data
        QString valError = crud.validateProjet(proj, false);
        if (!valError.isEmpty()) {
            // Show themed error dialog
            ErrorDialog errDlg(style(), "Erreur de validation", valError, this);
            errDlg.exec();
            return; // Do not save
        }

        // If validation passed, show success message and navigate back
        setWindowTitle("Gestion Projet");
        stack->setCurrentIndex(PROJ_LIST);
        QMessageBox::information(this, "Projet", "Projet enregistré avec succès!");
    });
