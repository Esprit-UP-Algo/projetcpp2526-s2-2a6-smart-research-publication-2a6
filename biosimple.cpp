...
static ExpStatus expStatusFromDb(const QString& s)
{
    const QString v = s.trimmed().toLower();
    if (v == "en cours") return ExpStatus::EnCours;
    if (v == "terminee" || v == "terminée" || v == "termine" || v == "terminé") return ExpStatus::Termine;
    if (v == "en attente" || v == "planifiee" || v == "planifiée" || v == "planifie" || v == "planifié") return ExpStatus::EnAttente;
    if (v == "suspendue" || v == "suspendu") return ExpStatus::Suspendue;
    return ExpStatus::EnCours;
}
    ...
    : QMainWindow(parent)
{
    ...
    eSearch->setPlaceholderText("Rechercher par statut, hypothese");
    ...
    QPushButton* eExportBtn = new QPushButton(st->standardIcon(QStyle::SP_DialogSaveButton), "  Exporter (PDF)");
    ...
    expTable->setHorizontalHeaderLabels({"Nom", "Hypothese", "Date début", "Date fin", "Statut"});
    ...
    auto setExpStatus = [=](int r, ExpStatus s, const QString& label){
        QTableWidgetItem* badge = new QTableWidgetItem(label);
        badge->setData(Qt::UserRole, (int)s);
        expTable->setItem(r, 4, badge);
    };

    auto setRowHeightAll = [=](){
        for(int r=0;r<expTable->rowCount();++r) expTable->setRowHeight(r, 46);
    };

    // Search by titre or hypothese
    ...
    eProjet->setFixedWidth(260);

    QDateEdit* eDateStart = new QDateEdit(QDate::currentDate());
    eDateStart->setCalendarPopup(true);
    eDateStart->setDisplayFormat("dd/MM/yyyy");
    eDateStart->setStyleSheet("QDateEdit{ background: rgba(255,255,255,0.65); border: 1px solid rgba(0,0,0,0.15); border-radius: 12px; padding: 10px 14px; color: rgba(0,0,0,0.65); font-weight: 900; }");

    QDateEdit* eDateEnd = new QDateEdit(QDate::currentDate().addDays(1));
    eDateEnd->setCalendarPopup(true);
    eDateEnd->setDisplayFormat("dd/MM/yyyy");
    eDateEnd->setStyleSheet(eDateStart->styleSheet());
    eDateEnd->setMinimumDate(eDateStart->date().addDays(1));

    QLabel* expErrorLeft = new QLabel;
    expErrorLeft->setWordWrap(true);
    expErrorLeft->setStyleSheet("color: #8B2F3C; font-weight: 700;");
    expErrorLeft->setVisible(false);

    QLabel* expErrorRight = new QLabel;
    expErrorRight->setWordWrap(true);
    expErrorRight->setStyleSheet("color: #8B2F3C; font-weight: 700;");
    expErrorRight->setVisible(false);
    ...
    e2LeftL->addWidget(expErrorLeft);
    ...
    e2RightL->addWidget(expRow(QStyle::SP_FileDialogInfoView, "Date début", eDateStart));
    e2RightL->addWidget(expRow(QStyle::SP_FileDialogInfoView, "Date fin", eDateEnd));
    e2RightL->addWidget(expRow(QStyle::SP_MessageBoxInformation, "Statut", eStatus));
    e2RightL->addWidget(expErrorRight);
    ...
    int* currentExpId = new int(-1);
    bool* isEditing = new bool(false);
    ...
    auto clearExpForm = [=](){
        ...
        eDateStart->setDate(QDate::currentDate());
        eDateEnd->setMinimumDate(eDateStart->date().addDays(1));
        eDateEnd->setDate(QDate::currentDate().addDays(1));
        expErrorLeft->clear();
        expErrorRight->clear();
        expErrorLeft->setVisible(false);
        expErrorRight->setVisible(false);
        expSave->setEnabled(false);
        *currentExpId = -1;
        *isEditing = false;
    };

    auto validateExpForm = [=](bool showPopup)->bool{
        QStringList leftErrors;
        QStringList rightErrors;

        if (eName->text().trimmed().isEmpty()) leftErrors << "Nom de l'experience requis.";
        if (eHypo->text().trimmed().isEmpty()) leftErrors << "Hypothese requise.";
        if (eProjet->currentIndex() <= 0 || !eProjet->currentData().isValid()) leftErrors << "Projet requis.";

        if (eStatus->currentIndex() <= 0) rightErrors << "Statut requis.";
        const QDate dDebut = eDateStart->date();
        const QDate dFin = eDateEnd->date();
        if (!dDebut.isValid()) rightErrors << "Date debut invalide.";
        if (!dFin.isValid()) rightErrors << "Date fin invalide.";
        if (dDebut.isValid() && dFin.isValid() && dFin <= dDebut) {
            rightErrors << "Date fin doit etre apres date debut.";
        }

        expErrorLeft->setVisible(!leftErrors.isEmpty());
        expErrorLeft->setText(leftErrors.join("\n"));
        expErrorRight->setVisible(!rightErrors.isEmpty());
        expErrorRight->setText(rightErrors.join("\n"));

        if (showPopup && (!leftErrors.isEmpty() || !rightErrors.isEmpty())) {
            QStringList allErrors = leftErrors;
            allErrors.append(rightErrors);
            QMessageBox::information(this, "Information", allErrors.join("\n"));
            return false;
        }

        return leftErrors.isEmpty() && rightErrors.isEmpty();
    };

    auto updateExpSaveState = [=](){
        expSave->setEnabled(validateExpForm(false));
    };
    ...
    auto loadProjects = [=](){
        ...
        QString err;
        if (!ExperienceCrud::loadProjects(items, &err)) {
            QMessageBox::warning(this, "Database", "Erreur chargement PROJET: " + err);
            return;
        }
        for (const auto& it : items) {
            eProjet->addItem(it.name, it.id);
        }
    };

    auto loadExperiences = [=](){
        expTable->setRowCount(0);
        QList<ExperienceRecord> rows;
        QString err;
        if (!ExperienceCrud::loadExperiences(rows, &err)) {
            QMessageBox::warning(this, "Database", "Erreur chargement EXPERIENCE: " + err);
            return;
        }
        int r = 0;
        for (const auto& rec : rows) {
            expTable->insertRow(r);
            const ExpStatus st = expStatusFromDb(rec.status);
            const QString statusLabel = rec.status.isEmpty() ? expStatusText(st) : rec.status;

            putE(r, 0, rec.titre);
            expTable->item(r, 0)->setData(Qt::UserRole, rec.id);
            putE(r, 1, rec.hypothese);
            putE(r, 2, rec.dateDebut.isValid() ? rec.dateDebut.toString("dd/MM/yyyy") : "");
            putE(r, 3, rec.dateFin.isValid() ? rec.dateFin.toString("dd/MM/yyyy") : "");
            setExpStatus(r, st, statusLabel);
            ++r;
        }
        setRowHeightAll();
    };
    ...
    QObject::connect(expEdit, &QPushButton::clicked, this, [=](){
        ...
        if (r < 0) {
            QMessageBox::information(this, "Information", "Veuillez sélectionner une ligne à modifier.");
            return;
        }
        QTableWidgetItem* idItem = expTable->item(r, 0);
        if (!idItem) return;
        *currentExpId = idItem->data(Qt::UserRole).toInt();
        *isEditing = true;

        ExperienceRecord rec;
        QString err;
        if (!ExperienceCrud::fetchExperience(*currentExpId, rec, &err)) {
            QMessageBox::warning(this, "Database", "Erreur chargement EXPERIENCE: " + err);
            return;
        }
        eName->setText(rec.titre);
        eHypo->setText(rec.hypothese);
        const QDate dStart = rec.dateDebut;
        const QDate dEnd = rec.dateFin;
        if (dStart.isValid()) eDateStart->setDate(dStart);
        if (dEnd.isValid()) eDateEnd->setDate(dEnd);
        eDateEnd->setMinimumDate(eDateStart->date().addDays(1));

        const QString s = rec.status.trimmed();
        int idx = eStatus->findText(s, Qt::MatchFixedString);
        if (idx < 0) idx = eStatus->findText(s, Qt::MatchContains);
        eStatus->setCurrentIndex(idx < 0 ? 0 : idx);

        const QVariant projId = rec.projetId;
        if (projId.isValid()) {
            int pidx = eProjet->findData(projId);
            if (pidx >= 0) eProjet->setCurrentIndex(pidx);
        }
    ...
    QObject::connect(expDel, &QPushButton::clicked, this, [=](){
        ...
        QString resume = QString("Expérience : %1 | Hypothese : %2")
        ...
        QString err;
        if (!ExperienceCrud::deleteExperience(id, &err)) {
            QMessageBox::warning(this, "Database", "Erreur suppression EXPERIENCE: " + err);
            return;
        }
        loadExperiences();
    });
    ...
    QObject::connect(expSave, &QPushButton::clicked, this, [=](){
        if (!validateExpForm(true)) return;

        const QString titre = eName->text().trimmed();
        const QString hypo = eHypo->text().trimmed();
        const QString status = eStatus->currentIndex() > 0 ? eStatus->currentText() : QString();
        const QDate dDebut = eDateStart->date();
        const QDate dFin = eDateEnd->date();
        const QVariant projId = eProjet->currentData();

        ExperienceRecord rec;
        rec.id = *currentExpId;
        rec.titre = titre;
        rec.hypothese = hypo;
        rec.dateDebut = dDebut;
        rec.dateFin = dFin;
        rec.status = status;
        rec.projetId = projId;

        QString err;
        if (*isEditing && *currentExpId > 0) {
            if (!ExperienceCrud::updateExperience(rec, &err)) {
                QMessageBox::warning(this, "Database", "Erreur mise a jour EXPERIENCE: " + err);
                return;
            }
        } else {
            if (!ExperienceCrud::insertExperience(rec, &err)) {
                QMessageBox::warning(this, "Database", "Erreur insertion EXPERIENCE: " + err);
                return;
            }
        }
    ...
    QObject::connect(eName, &QLineEdit::textChanged, this, [=](){ updateExpSaveState(); });
    QObject::connect(eHypo, &QLineEdit::textChanged, this, [=](){ updateExpSaveState(); });
    QObject::connect(eStatus, &QComboBox::currentIndexChanged, this, [=](int){ updateExpSaveState(); });
    QObject::connect(eProjet, &QComboBox::currentIndexChanged, this, [=](int){ updateExpSaveState(); });
    QObject::connect(eDateStart, &QDateEdit::dateChanged, this, [=](const QDate& d){
        eDateEnd->setMinimumDate(d.addDays(1));
        if (eDateEnd->date() <= d) eDateEnd->setDate(d.addDays(1));
        updateExpSaveState();
    });
    QObject::connect(eDateEnd, &QDateEdit::dateChanged, this, [=](){ updateExpSaveState(); });
    // ==========================================================
...
QPushButton:hover{ background: %2; }
    ...
    QLabel* expDetHypo = nullptr;
    QLabel* expDetDateStart = nullptr;
    QLabel* expDetDateEnd = nullptr;
    ...
    expDetailsL->addWidget(expDetailRow("Hypothese", expDetHypo));
    expDetailsL->addWidget(expDetailRow("Date debut", expDetDateStart));
    expDetailsL->addWidget(expDetailRow("Date fin", expDetDateEnd));
    ...
    auto updateExpDetailsFromRow = [=]()->bool{
        ...
        expDetHypo->setText(expTable->item(r,1)->text());
        expDetDateStart->setText(expTable->item(r,2)->text());
        expDetDateEnd->setText(expTable->item(r,3)->text());
