...
AiBubble::~AiBubble()
{
    // Déconnecter avant stop() pour éviter tout callback sur un objet en cours de destruction
    if (m_tts) {
    ...
    : QFrame(parent, Qt::Tool | Qt::FramelessWindowHint | Qt::WindowStaysOnTopHint)
{
    ...
    connect(closeBtn, &QPushButton::clicked, this, [this](){
        // Arrêter la TTS directement (synchrone) AVANT de cacher le widget.
        // Le timer différé de requestSpeechStop() déclenchait stop() après setVisible(false),
        // ce qui crashait le backend SAPI/WinRT de Windows.
        // QueuedConnection sur stateChanged empêche déjà la ré-entrance.
        resetSpeechUi();
