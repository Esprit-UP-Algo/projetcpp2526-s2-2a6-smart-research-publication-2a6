...
protected:
    void paintEvent(QPaintEvent*) override {
        ...
        int leftPad = 40;
        int bottomPad = 50;  // Increased to show labels better
        ...
        const QColor bar1Clr  = m_dark ? QColor("#00D9FF") : QColor("#6A4FD9");  // Vibrant gradient
        const QColor bar2Clr  = m_dark ? QColor("#007EFF") : QColor("#00D9FF");  // Bright blue/cyan
        ...
        for (int i=0; i<n; ++i) {
            ...
            p.setPen(textClr);
            p.setFont(f);
            // Draw label with more space below bar
            int labelY = plot.bottom() + 8;
            p.drawText(QRect(bar.left() - 10, labelY, bar.width() + 20, 22),
                       Qt::AlignCenter | Qt::AlignTop, m_bars[i].label);
        }
    }
