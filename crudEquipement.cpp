...
bool isLettersOnlyEquipmentText(const QString& value)
{
    static const QRegularExpression re(QStringLiteral("^[\\p{L}\\s'\\-]+$"));
...
bool EquipementCrud::insertEquipement(const EquipementRecord& in, QString* error)
{
    ...
    if (!isLettersOnlyEquipmentText(nomEq)) {
        if (error) *error = "nom_equipement doit contenir uniquement des lettres.";
        return false;
    }

    if (!fabEq.isEmpty() && !isLettersOnlyEquipmentText(fabEq)) {
        if (error) *error = "fabricant doit contenir uniquement des lettres.";
        return false;
    }
    ...
    const bool hasIdExp = equipementHasIdExpColumn();
    if (hasIdExp) {
        if (in.idExp.isNull() || !in.idExp.isValid() || in.idExp.toInt() <= 0) {
            if (error) *error = "Id_exp est obligatoire pour l'équipement.";
            return false;
        }
        QString checkErr;
